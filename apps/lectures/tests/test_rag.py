...
class TestRetrieveContext:
    def test_returns_empty_string_when_no_chunks(self, mocker):
        mock_collection = mocker.patch("apps.lectures.rag.collection")
        mock_collection.get.return_value = {"ids": []}

        assert retrieve_context(lecture_id=1, question="what is x?") == ""
        mock_collection.query.assert_not_called()

    def test_clamps_top_k_to_available_chunk_count(self, mocker):
        mock_collection = mocker.patch("apps.lectures.rag.collection")
        mock_collection.get.return_value = {"ids": ["1_0", "1_1"]}
        mock_collection.query.return_value = {"documents": [["chunk a", "chunk b"]]}

        result = retrieve_context(lecture_id=1, question="what is x?", top_k=5)

        mock_collection.query.assert_called_once_with(
            query_texts=["what is x?"], n_results=2, ids=["1_0", "1_1"],
        )
