...
class TestStoreChunks:
    def test_deletes_existing_then_adds(self, mocker):
        mock_collection = mocker.patch("apps.lectures.ai_services.collection")

        store_chunks(42, ["chunk one", "chunk two"])

        mock_collection.delete.assert_called_once_with(where={"lecture_id": "42"})
        mock_collection.add.assert_called_once_with(
            documents=["chunk one", "chunk two"],
            ids=["42_0", "42_1"],
            metadatas=[{"lecture_id": "42"}, {"lecture_id": "42"}],
        )

    def test_wraps_failures_in_runtime_error(self, mocker):
        mock_collection = mocker.patch("apps.lectures.ai_services.collection")
        mock_collection.add.side_effect = Exception("chroma down")

        with pytest.raises(RuntimeError, match="Failed to store chunks for lecture 7"):
